...
export class WorkitemComponent implements OnInit {
  ...
  constructor(
    ...
    this.changeSubscription = this.gchservice
      .getMessage()
      .subscribe((message) => {
        this.changeMessage = message;

        this.updateState(
          this.changeMessage.ref,
          this.changeMessage.value,
          this.changeMessage.caseID,
          this.changeMessage.refType
        );
      });

    this.actionSubscription = this.gactionsservice
      .getMessage()
      .subscribe((message) => {
        this.actionMessage = message;

        this.handleThisFormActions(
          this.actionMessage.actionName,
          this.actionMessage.action,
          this.actionMessage.caseID,
          this.actionMessage.reference
        );
      });

    this.refreshAssignmentSubscription = this.raservice
      .getMessage()
      .subscribe((message) => {
        this.refreshAssignmentMessage = message;

        this.handleRefreshAssignmentActions(
          this.refreshAssignmentMessage.action,
          this.refreshAssignmentMessage.data
        );
      });

    this.getNewCaseSubscription = this.gncservice
      .getMessage()
      .subscribe((message) => {
    ...
    this.getRecentSubscription = this.grservice
      .getMessage()
      .subscribe((message) => {
    ...
    this.pageInstructionsSubscription = this.piservice
      .getMessage()
      .subscribe((message) => {
        this.pageInstructionsMessage = message;

        this.handlePageInstructions(
          message.caseID,
          message.refType,
      ...
      switch (sAction) {
        ...
        case "runScript":
          ...
          try {
            ...
            console.log(
              "RunScript can't eval: " + sScript + ", function may not exist."
            );
          }
        ...
        case "localAction":
          this.localAction(
            oAction.actionProcess.localAction,
            oAction.actionProcess.target,
            oAction.actionProcess.customTemplate
          );
          break;
        default:
  ...
  handleRefreshAssignmentActions(sAction: string, oAction: any) {
    switch (sAction) {
      case "addRow":
        ...
        let targetAdd = this.refHelper.getRepeatFromReference(
          oAction.layoutData.reference,
          oAction.layoutData.referenceType,
          postContentAdd
        );
      ...
      case "removeRow":
        ...
        let targetRemove = this.refHelper.getRepeatFromReference(
          oAction.layoutData.reference,
          oAction.layoutData.referenceType,
          postContentRemove
        );
  ...
  handlePageInstructions(
    ...
    if (sCaseID == this.currentCaseID$) {
      if (sRefType == "List") {
        ...
        this.pageinstructions.addAListInstruction(
          sInstructions,
          sTarget,
          nIndex,
          oContent
        ...
        this.pageinstructions.addAGroupInstruction(
          sInstructions,
          sTarget,
          sIndex,
          oContent
    ...
    this.aservice.getAssignment(assignmentID).subscribe(
      ...
      (assignmentError) => {
        let snackBarRef = this.snackBar.open(
          "Errors from get assignment:" + assignmentError.errors,
          "Ok"
        );
      }
    );
    ...
    this.aservice.getFieldsForAssignment(assignmentID, action).subscribe(
      (response) => {
        ...
        if (this.bIsLocalAction) {
          this.currentAssignmentFields$[
            "view"
          ] = this.refHelper.updateViewWithLocalState(
            response.body.view,
            this.state
          );
        }
      ...
      (err) => {
        let snackBarRef = this.snackBar.open(
          "Errors from assignment:" + err.message,
          "Ok"
        );
      }
    );
  ...
  getLocalAction(assignmentID: string, action: string, caseID: string) {
    ...
    this.aservice.getFieldsForAssignment(assignmentID, action).subscribe(
      (response) => {
        ...
        let localView = this.refHelper.updateViewWithLocalState(
          response.body.view,
          this.state
        );

        const dialogRef = this.localActionDialog.open(
          LocalactiondialogComponent,
          {
            width: "550px",
            data: {
              dialogView: localView,
  ...
  getCase(caseID: string) {
    ...
    this.cservice.getCase(caseID).subscribe(
      (response) => {
        console.log("CASETYPE ID", caseID);
  ...
  getNewCase(caseID: string) {
    this.cservice.getCaseCreationPage(caseID).subscribe(
      ...
      (err) => {
        let snackBarRef = this.snackBar.open(
          "Errors from create case:" + err.message,
          "Ok"
        );
      }
    );
  ...
  getView(oAssignment: any) {
    ...
    if (this.bRefreshOccurred) {
      ...
      this.state = this.refHelper.getInitialValuesFromView(
        this.currentView$,
        this.bUseRepeatPageInstructions,
        this.bUsePagePageInstructions
      );
    ...
    if (this.currentView$["validationMessages"] != "") {
      let snackBarRef = this.snackBar.open(
        this.currentView$["validationMessages"],
        "Ok"
      );
    }
  }
  ...
  getPage(oPage: any) {
    let timer = interval(10).subscribe(() => {
      this.gpservice.sendMessage(oPage.name, oPage.pageID, oPage);
      timer.unsubscribe();
    });
  ...
  updateState(sRef: string, sValue: string, sCaseID: string, sRefType: string) {
    ...
    if (sCaseID === this.currentCaseID$) {
      if (this.bUseRepeatPageInstructions) {
        if ((sRefType != null && sRefType == "List") || sRefType == "Group") {
      ...
      if (this.bUsePagePageInstructions) {
        if (sRefType == null && sRef.indexOf(".") > 0) {
          this.updateEmbeddedPageInstructionState(
            sRef,
            sValue,
            sCaseID,
            sRefType
  ...
  updateEmbeddedPageInstructionState(
    sRef: string,
    sValue: string,
    sCaseID: string,
    sRefType: string
    ...
    this.pageinstructions.addAnUpdatePageInstruction(
      oTargetInfo["pageName"],
      oTargetInfo["propName"],
      sValue
    );
  }

  updatePageInstructionState(
    sRef: string,
    sValue: string,
    sCaseID: string,
    sRefType: string
    ...
    if (sRefType == "List") {
      ...
      bHasLast = this.pageinstructions.isLastListInstruction(
        "UPDATE",
        oTargetInfo["target"],
        nIndex
      );
    } else if (sRefType == "Group") {
      bHasLast = this.pageinstructions.isLastGroupInstruction(
        "UPDATE",
        oTargetInfo["target"],
        oTargetInfo["index"]
      );
  ...
  updateAssignmentView(sRef: string, sValue: string) {
    if (this.currentAssignmentFields$) {
      let fieldNode = this.refHelper.customUpdateJSON(
        this.currentAssignmentFields$,
        "reference",
        sRef
      );
  ...
  refreshView(stateData: any, oAction: any) {
    ...
    this.aservice
      .performRefreshOnAssignment(
        ...
        sRefreshFor,
        stateData,
        this.pageinstructions.getPageInstructions()
      )
      .subscribe(
        (response) => {
  ...
  handleValidationErrors(validationMessages: Array<any>) {
    ...
    for (message of validationMessages) {
      ...
      if (ref) {
        ...
        let controlName = this.refHelper.getControlNameFromReference(
          ref,
          this.tvComp.groups$
        );
        ...
        if (this.tvComp.fg.controls[controlName]) {
          this.tvComp.fg.controls[controlName].setErrors(
...
export class LocalactiondialogComponent implements OnInit {
  constructor(
    public localActionDialogRef: MatDialogRef<LocalActionDialog>,
  ...
  submitDialog() {
    ...
    this.gactionsservice.sendMessage(
      "refresh",
      { action: "localAction" },
      this.data.caseID,
      null

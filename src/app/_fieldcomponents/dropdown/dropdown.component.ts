...
export class DropdownComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.fieldComp.control.modes.length > 0) {
      this.tooltip$ = this.refHelper.htmlDecode(
        this.fieldComp.control.modes[0].tooltip
      );
    }
    ...
    if (this.noLabel) {
      ...
      if (this.fieldComp.label != "") {
        ...
        this.fieldComp.label == "" &&
        this.fieldComp.labelReserveSpace
      ) {
        this.showLabel$ = true;
      }
    }
    ...
    let bUseLocalOptionsForDataPage =
      localStorage.getItem("useLocalOptionsForDataPage") == "true"
        ? true
        : false;
    let bUseLocalOptionsForClipboardPage =
    ...
    if (this.fieldComp.control.modes[0].listSource === "datapage") {
      ...
      if (this.fieldComp.control.modes[0].loadMode != null) {
        bHaveLocalOption =
          this.fieldComp.control.modes[0].loadMode.toLowerCase() == "auto"
            ? true
            : false;
      }
      ...
      if (bUseLocalOptionsForDataPage && bHaveLocalOption) {
        ...
        // handle data page
        let dataPageName =
          this.fieldComp.control.modes[0].dataPageID ||
          this.fieldComp.control.modes[0].dataPage;
      ...
      if (bUseLocalOptionsForClipboardPage && bHaveLocalOption) {
        ...
        this.getCaseSubscription = this.gcservice
          .getMessage()
          .subscribe((message) => {
            ...
            if (message) {
              ...
              if (workPage) {
                ...
                if (cPage) {
                  ...
                  this.valueReadonly$ = this.getOptionValue(
                    this.fieldComp.value
                  );
                }
              }
            }
  ...
  fieldChanged(e) {
    ...
    this.gchservice.sendMessage(
      this.fieldComp.reference,
      e.value,
      this.CaseID,
      this.RefType$
    );

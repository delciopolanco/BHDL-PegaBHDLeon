...
export class MaintabsComponent implements OnInit {
  ...
  constructor(
    ...
    this.closeWorkSubscription = this.cwService
      .getMessage()
      .subscribe((message) => {
    ...
    this.openNewCaseSubscrption = this.oncService
      .getMessage()
      .subscribe((message) => {
    ...
    this.renameTabSubscription = this.rtService
      .getMessage()
      .subscribe((message) => {
        this.renameTabMessage = message;

        this.renameTab(
          this.renameTabMessage.tabName,
          this.renameTabMessage.newTabName
        );
      });

    this.openRecentSubscription = this.orService
      .getMessage()
      .subscribe((message) => {
    ...
    if (this.message) {
      if (!this.isNewCase) {
        this.gaService.sendMessage(
          this.message.assignment.pxRefObjectInsName,

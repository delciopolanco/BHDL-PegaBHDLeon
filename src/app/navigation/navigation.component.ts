...
export class NavigationComponent implements OnInit {
  @Input() isPlatformMobile: boolean = false;
  ...
  ngOnInit() {
    ...
    this.progressSpinnerSubscription = this.psservice
      .getMessage()
      .subscribe((message) => {
  ...
  showSettings() {
    this.bUseNewRow =
      localStorage.getItem("useNewRow") == "true" ? true : false;
    ...
    this.bUseLocalOptionsForDataPage =
      localStorage.getItem("useLocalOptionsForDataPage") == "true"
        ? true
        : false;
    this.bUseLocalOptionsForClipboardPage =
      localStorage.getItem("useLocalOptionsForClipboardPage") == "true"
        ? true
        : false;
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result != undefined) {
        ...
        localStorage.setItem(
          ...
          result.bUseRepeatPageInstructions.toString()
        );
        localStorage.setItem(
          "usePagePageInstructions",
          result.bUsePagePageInstructions.toString()
        );
        localStorage.setItem(
          "useLocalOptionsForDataPage",
          result.bUseLocalOptionsForDataPage.toString()
        );
        localStorage.setItem(
          "useLocalOptionsForClipboardPage",
          result.bUseLocalOptionsForClipboardPage.toString()
        );
      }
    });
  }
...
export class SettingsdialogComponent implements OnInit {
  ...
  closeDialog() {
    this.dialogRef.close({
      ...
      bUseLocalOptionsForClipboardPage: this.data
